...
class Roster
{
	public:
		...
		char calculateLetterGrade( int totalScore );

		void displayStudent( std::string cNumber );

		void displayStudentsInfo(void);
	...
	private:
		...
		static const int MAX_COLS = 9;				// there will never be more than 9 fields to enter
		std::string			m_courseName;			// THe name of the course
		int					m_studentNum;			// Actual # of Students
		Student				m_students[MAX_NUM];	// The array of student objects

		static const int TABLE_COLUMN_WIDTH = 7;
};
#endif
